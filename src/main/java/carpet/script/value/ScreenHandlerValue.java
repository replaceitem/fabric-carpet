...
public class ScreenHandlerValue extends Value {
    ...
    public void showScreen(PlayerEntity player) {
        if(!(screenHandler instanceof NamedScreenHandlerFactory)) throw new InternalExpressionException("Not a NamedScreenHandler, how can this happen???");
    ...
    public ScreenHandlerFactory createGenericScreenHandlerFactory(ScreenHandlerType<?> screenHandlerType, Text displayName) {
        ...
        return new SimpleNamedScreenHandlerFactory((i, playerInventory, playerEntity) -> {
            ...
            screenHandler.setPlayerRestriction(playerEntity,!this.restricted);
            if (functionValue != null) addClickListener(screenHandler,playerEntity);
            return screenHandler;
        },displayName);
    }

    public ScreenHandlerFactory createHopperScreenHandlerFactory(Text displayName) {
    ...
    public ScreenHandlerFactory createAnvilScreenHandlerFactory(Text displayName) {
        ...
        return new SimpleNamedScreenHandlerFactory((i, playerInventory, playerEntity) -> {
            ...
            for(int j = 0; j < inventory.size(); j++) {
                System.out.println(inventory.getStack(j).toString());
    ...
    public ScreenHandlerFactory createLecternScreenHandlerFactory(Text displayName) {
    ...
    private void addClickListener(ScreenHandler screenHandler, PlayerEntity playerEntity) {
        FunctionArgument<Value> functionArgument = FunctionArgument.fromCommandSpec(context.host, functionValue);
        if(functionArgument.function.getNumParams() != 4) throw new InternalExpressionException("Callback function requires four parameters");
        ...
        screenHandler.addListener(new ScreenHandlerScarpetListener() {
            ...
            public void onSlotClick(ScreenHandler handler, int slotId, ItemStack stack, String type) {
                ...
                functionArgument.function.callInContext(context, Context.VOID, FunctionValue.lazify(Arrays.asList(playerValue, slotValue, stackValue,typeValue)));
            }
    ...
    public static Inventory getInvetoryForScreenHandler(ScreenHandlerType<?> screenHandlerType) {
